...
| 非公開 | `dotfiles-private` | 個人用メールアドレス、SSH、非公開の AI 設定、宣言的なリンク対応表 |
...
privateリポジトリにはリンク元・リンク先・OS条件だけを `links.conf` へ宣言し、設定を
実行可能コードとして読み込まない。リンクの作成・検査・削除ロジックは公開リポジトリが所有する。
