...
- `.docx` への実際の変更履歴・コメントの追加は、その場でPythonスクリプトを書かず `docx-redline` コマンド（`bin/unix/docx-redline`、実体は [y-marui/python-docx-redline](https://github.com/y-marui/python-docx-redline) を `uvx` 経由で実行）のサブコマンドで行う。`docx-redline inspect` で編集前の段落・既存コメント・変更履歴を確認し、既存変更履歴の承諾には `accept-revisions`、編集には `replace`（一致1件が既定、複数箇所は `--occurrence`/`--all`）、`replace-batch`（複数の置換をJSONでまとめて適用）、`replace-paragraph`、`insert-paragraph`、`add-comment`、`strip-comments`、`strip-format-revisions` を用いて編集し、納品前に `validate` で安全確認する。コメント・変更履歴の著者名は、実行中のコードエージェント名を `--author` で明示して用いる。名前を確認できない場合、汎用の既定名で記録せず、ユーザーに確認する。既存のコメント著者になりすましてはならない。
...
Deep-Auto では、表記・文法の修正だけで完了とせず、対象範囲の目的・問い・方法・準備・実行可能性（文書の性質に応じた対応項目）のつながりを読者が追えるか確認する。既存の関係を意味を変えずに明確化できる接続の補強、段階関係の明示、段落順の整理は変更履歴で適用する。判定基準、目標値、事実的・科学的根拠、実現可能性の証拠、主張の取捨選択など著者の専門判断を要する論点はアンカーコメントにし、納品時には構成上の編集とコメントに留めた論点を区別して報告する。

### Research and grant proposal review

研究計画書・助成金申請書を `Deep` または `Deep-Auto` でレビューする際は、通常の論理・構成レビューに加え、読者が研究の意義と実行可能性を短時間で把握できるかを確認する。次の問題は著者の専門的・戦略的判断を伴うため、本文を自動で書き換えず、具体的な改善方向を示すアンカーコメントにする。

- 学術的「問い」が既存の説明を不必要に否定する二者択一になっていないか。複数の機構・要因が関わり得る場合は、各要因の寄与・関係を明らかにして理解を前進させる問いとして示せるか確認する。
- 研究の中核となる制御量、変化させる物理量・条件、評価対象、得られる判断の因果関係を、概念図だけで直感的に追えるか。図が予定されている場合は、図に担わせるべき関係を具体的に指摘する。
- 予備結果がある場合、各結果が研究計画のどの鍵となる工程・能力を既に確立している証拠なのか、また後続の実験をなぜ実行可能にするのかが明記されているか。
- 研究動向が文献や個別結果の列挙でなく、既に分かっていること、未解決のこと、本研究の位置づけを俯瞰的に示しているか。差異の説明に不釣り合いな長さを費やしている場合は、圧縮の余地を指摘する。
- 概要、本文、図表・キャプションを通じて、研究対象、応用先、略語などの基本用語が一貫しているか。

コメントは「何が読者に伝わらないか」と「どの関係・役割を明示すべきか」を短く述べ、事実・科学的結論・引用を補わない。図の作成、本文の圧縮、問いの採否は著者の判断として残す。
