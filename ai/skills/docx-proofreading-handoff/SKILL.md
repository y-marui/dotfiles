...
1. 対象文書、同階層の `PROOFREADING.md`、`PROOFREADING.docx.md`、および `PROOFREADING.md` 先頭の有効な `@<relative-path>` 参照を読む。`@` 参照は `docx-proofreading` と同じ規則で `$OBSIDIAN_ROOT/writing/<relative-path>` に解決する。`PROOFREADING.docx.md` 内の `@` 行は外部参照にしない。
...
| DOCXだけに関わる特定文書・フォルダの用語、変更履歴、組版、著者が確定した判断 | 同階層の `PROOFREADING.docx.md` |
...
| DOCX固有の安全な編集不変条件、ツール利用手順 | `docx-proofreading/SKILL.md` の更新案 |
| 特定の文書種別(科学的文書など)だけに関わる、案件・著者に依存しない内容レビューの観点 | `docx-proofreading/references/<domain>.md` の更新案 |
...
1. まず対象文書、`PROOFREADING.md`、`PROOFREADING.docx.md`、参照済みのObsidianガイド、skillを確認し、反映先が明確な情報は重複・競合を避けて最小限追記する。既存の指示を上書き・要約して失わせない。
...
3. `PROOFREADING.md`、`PROOFREADING.docx.md` とObsidianガイドには、今後も判断を変える規則だけを書く。今回の作業量、途中経過、個別本文、未確定の解釈は入れない。
...
6. すべての案件で有用になり得る規則を見つけた場合は、DOCX・PPTX共通なら `_proofreading-common/review-principles.md`、DOCX固有なら `docx-proofreading/SKILL.md`、特定の文書種別だけに関わる観点なら `docx-proofreading/references/<domain>.md` の更新候補を、根拠・追加先・短い文案とともに報告する。ユーザーが変更まで明示的に依頼した場合だけ、対象ファイルを更新して検証する。それ以外は提案に留める。
