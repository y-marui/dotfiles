# dev-charter Update Checklist (lite)
...
`git subtree pull`（`lite` ブランチ）で dev-charter を更新した後、以下のプロンプトを AI に貼り付けて実行してください。
...
> 変更ファイルに限らず全ファイルを精査したい場合は [インストールチェックリスト](https://github.com/y-marui/dev-charter/blob/lite/INSTALL_CHECKLIST.md) を使用してください。

## English

After running `git subtree pull` (the `lite` branch) to update dev-charter in your project, paste the following prompt into your AI tool.
...
> For a full review of all charter files (not just changed ones), use the [Install Checklist](https://github.com/y-marui/dev-charter/blob/lite/INSTALL_CHECKLIST.md).
